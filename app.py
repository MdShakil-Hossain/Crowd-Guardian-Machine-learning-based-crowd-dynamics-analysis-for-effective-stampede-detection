...
def render_results(df_frames, df_events, labeled_path, key_seed=None):
    ...
    if not df_frames.empty:
        ...
        st.info("No frame data available for charts.")
...
def add_boxes_to_video(input_path, output_path, df_events, snapshots):
    ...
    while True:
        ...
        if prev_gray is not None:
            ...
            transform = np.matmul(transform, delta_transform)
        prev_gray = gray.copy()
        ...
        if f in frame_to_event:
            ...
            if eid in event_boxes:
                ...
                pts = np.array([[x0, y0, 1], [x1, y1, 1]], dtype=np.float32)
                inv_transform = np.linalg.inv(transform)
                transformed_pts = np.matmul(pts, inv_transform.T)[:, :2]
...
if go:
    if not model:
        ...
        st.write(f"Debug: Input video path: {tmp.name}")
        with st.spinner("Analyzing video…"):
            try:
                df_frames, df_events, prelim_labeled_path = analyze_video(
                    tmp.name, model, detection_mode=detection_mode
                )
                st.write(f"Debug: Analysis complete. Frames: {len(df_frames)}, Events: {len(df_events)}")
            except Exception as e:
        ...
        try:
            ...
            st.write(f"Debug: Boxed video path: {boxed_path}, Size: {os.path.getsize(boxed_path) if os.path.exists(boxed_path) else 0} bytes")
        except Exception as e:
            st.error(f"Failed to add boxes to video: {str(e)}")
            boxed_path = prelim_labeled_path
        ...
        st.write(f"Debug: Snapshots generated: {len(st.session_state['video_xai']['snapshots'])}")
