# app.py — Crowd Guardian (Video + Snapshots)
...
# --- IMPORTANT: enable video output ---
SNAPSHOT_ONLY = False  # was True; now also write labeled video

# ---------- Snapshot/video overlay control ----------
SHOW_ZONE_BOX = True  # draw the red zone box + caption on video (matches Phase-1 look)
...
def build_blob_detector(
    frame_w, frame_h, min_frac=MIN_FRAC, max_frac=MAX_FRAC, min_circ=MIN_CIRC, min_iner=MIN_INER, thresh_step=10
):
    ...
    return cv2.SimpleBlobDetector(p) if int(cv2.__version__.split(".")[0]) < 3 else cv2.SimpleBlobDetector_create(p)
...
def combine_labels(y_heads, y_cnn, rule=COMBINE_RULE):
    ...
    if rule == "and":
        return 1 if (y_heads and y_cnn) else 0
    if rule == "or":
        return 1 if (y_heads or y_cnn) else 0
    if rule == "cnn_only":
        return int(y_cnn)
    if rule == "heads_only":
        return int(y_heads)
...
def transcode_to_h264(src_path: str, dst_path: str, fps: float):
    ...
    cmd = [
        ...
        src_path,
        "-r",
        str(int(round(fps if fps and fps > 0 else 25))),
        "-c:v",
        "libx264",
        "-pix_fmt",
        "yuv420p",
...
""",
    ...
    with c1:
        st.download_button(
            ...
            key=f"dl_events_{uid}",
        )
    with c2:
        st.download_button(
            "⬇️ frame_preds.csv",
            df_frames.to_csv(index=False).encode("utf-8"),
            file_name="frame_preds.csv",
            mime="text/csv",
            use_container_width=True,
            key=f"dl_frames_{uid}",
        )
    with c3:
        if labeled_path and os.path.exists(labeled_path) and os.path.getsize(labeled_path) > 0:
            with open(labeled_path, "rb") as fh:
                st.download_button(
                    ...
                    mime="video/mp4" if labeled_path.lower().endswith(".mp4") else "video/avi",
                    use_container_width=True,
                    key=f"dl_video_{uid}",
                )
        else:
    ...
    if isinstance(df_events_zones, pd.DataFrame) and not df_events_zones.empty:
        st.download_button(
            ...
            file_name="events_zones.csv",
            mime="text/csv",
            use_container_width=True,
            key=f"dl_events_z_{uid}",
        )
    ...
    if snapshots:
        ...
        for s in snapshots:
            ...
            caption = (
                ...
                f"{s.get('timecode','?')} • {s.get('zone_id','?')} (risk {risk:.2f})"
            )
        ...
        st.download_button(
            "⬇️ event_snapshots.csv",
            df_snaps.to_csv(index=False).encode("utf-8"),
            file_name="event_snapshots.csv",
            mime="text/csv",
            use_container_width=True,
            key=f"dl_snaps_csv_{uid}",
        )
...
if "video_results" in st.session_state:
    ...
    render_results(_res["df_frames"], _res["df_events"], _res.get("labeled_path"), key_seed=st.session_state.get("render_nonce"))
...
def update_tracks(tracks, detections, radii, max_dist, window_cap):
    ...
    for j in un_curr:
        ...
        r = float(radii[j])
        survivors.append(
            {"pos": p, "r": r, "miss": 0, "down_streak": 0, "hist": deque([(p[0], p[1], r)], maxlen=window_cap + 2)}
        )
    return survivors

# =============================================================================
# Core analysis (VIDEO ENABLED)
# =============================================================================
def analyze_video(
    video_path,
    model,
    ...
    if not writer.isOpened():
        # fallback: try mp4v direct
        alt_fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        writer = cv2.VideoWriter(mp4_path, alt_fourcc, float(max(1.0, eff_fps)), (W, H))

    wrote_to_mp4_direct = writer.isOpened() and writer.filename if hasattr(writer, "filename") else False

    labeled_path = ""  # will be set after processing
...
''',
    ...
    # helper to save the best snapshot per event — draw overlays on the saved image
    def save_current_best():
        ...
        # draw the zone + caption on the snapshot
        ...
        cv2.putText(
            frame, label, (max(6, x0), max(24, y0 - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA
        )
        ...
        if f % step == 0:
            ...
            if prev_gray_for_metrics is not None and prev_gray_for_metrics.shape == gray.shape:
                try:
                    ...
                    cond_speed = (flow_baseline.ready and flow_fast_frac >= FLOW_FAST_FRAC_MIN) or (
                        flow_p95 >= FLOW_P95_MIN
            ...
            if XAI_ENABLED:
                ...
                grid_boxes_local, cell_w, cell_h = make_grid(W, H, rows=GRID_ROWS, cols=GRID_COLS)
                cell_records = []
                for ((x0, y0, x1, y1), cid) in grid_boxes_local:
                    cell_records.append(
                        {
                            "cell_id": cid,
                            "x0": x0,
                            ...
                            "sum_dy_norm": 0.0,
                            "max_dy_norm": 0.0,
                            "torso_flow_accum": 0.0,
                            "cnn_cell": 0.0,
                            "heads": 0,
                        }
                    )

                def cell_index_for(xp, yp):
                    c = min(GRID_COLS - 1, max(0, int(xp // max(1, cell_w))))
                    r = min(GRID_ROWS - 1, max(0, int(yp // max(1, cell_h))))
                    return r * GRID_COLS + c

                def neighbors_y_median(xh, yh, r_head):
                    ys = []
                    for t2 in tracks:
                        (x2, y2) = t2["pos"]
                        r2 = max(2.0, t2.get("r", r_head))
                        if abs(x2 - xh) <= NEIGH_RADIUS_MULT * r_head and abs(y2 - yh) <= 3.0 * r_head:
                            ys.append(y2)
                    if len(ys) < 3:
                        ys.extend([yh] * (3 - len(ys)))
                    ys.sort()
                    return ys[len(ys) // 2]
                ...
                for tinfo in tracks:
                    ...
                    x0r = int(max(0, xh - 1.2 * rhead))
                    x1r = int(min(W, xh + 1.2 * rhead))
                    yh0 = int(max(0, yh - 1.0 * rhead))
                    yh1 = int(min(H, yh + 0.5 * rhead))
                    yt0 = int(max(0, yh + 0.5 * rhead))
                    yt1 = int(min(H, yh + 3.0 * rhead))
                ...
                # choose best zone for overlay + snapshot tracking
                cands = [i for i, rc in enumerate(cell_records) if rc["cand"] > 0]
                best_i = (
                    max(cands, key=lambda i: cell_records[i]["risk"])
                    if cands
                    else max(range(len(cell_records)), key=lambda i: cell_records[i]["risk"])
                )
                best = cell_records[best_i]
                bx0, by0, bx1, by1 = best["x0"], best["y0"], best["x1"], best["y1"]
                best_risk = best["risk"]
                # prepare overlay label (only if we end up in an event frame)
                overlay_bbox = (bx0, by0, bx1, by1)
                # overlay_text is set after event id is known below

                # keep best snapshot frame within the event
                if (current_best is None) or (best_risk > float(current_best.get("risk_score", -1))) or (
                    current_best and current_best.get("event_id") != event_id
                ):
                    current_best = {
                        "event_id": event_id,
            ...
            frames_rows.append(
                (
                    ...
                    y_cnn,
                    y_heads,
                    1 if any_head_and_torso_down else 0,
                    flow_mean,
                    flow_p95,
                    flow_coh,
                    flow_div_out,
            ...
            if final_label == 1 and not in_event:
                ...
                if dur_frames >= min_event_frames:
                    end_t = (f - 1) / fps
                    events_rows.append(
                        (start_f, f - 1, start_t, end_t, sec_to_tc(start_t), sec_to_tc(end_t), end_t - start_t)
                    )
                    if current_best:
                        save_current_best()
                in_event, start_f, start_t = False, None, None
            ...
            if final_label == 1 and SHOW_ZONE_BOX and overlay_bbox is not None:
                x0, y0, x1, y1 = overlay_bbox
                cv2.rectangle(frame_out, (x0, y0), (x1, y1), (0, 0, 255), 3)
                # Use latest best risk if available
                risk_txt = ""
                if current_best is not None and "risk_score" in current_best:
                    risk_txt = f" • risk {float(current_best['risk_score']):.2f}"
                cv2.putText(
                    frame_out,
                    f"Event {event_id} • {tc}{risk_txt}",
                    (max(6, x0), max(24, y0 - 6)),
    ...
    # Decide labeled_path (prefer mp4)
    labeled_path_candidate = None
    if os.path.exists(mp4_path) and os.path.getsize(mp4_path) > 0 and writer is not None and writer.isOpened():
        labeled_path_candidate = mp4_path  # rare case if OpenCV wrote mp4 directly
    else:
        # If we wrote AVI, try ffmpeg transcode to MP4
        if os.path.exists(raw_path) and os.path.getsize(raw_path) > 0:
            labeled_src, ok, _ = transcode_to_h264(raw_path, mp4_path, eff_fps)
            labeled_path_candidate = labeled_src if ok else raw_path
        elif os.path.exists(mp4_path) and os.path.getsize(mp4_path) > 0:
            labeled_path_candidate = mp4_path

    labeled_path = labeled_path_candidate or ""
...
if go:
    if not model:
        ...
        with st.spinner("Analyzing video…"):
            df_frames, df_events, labeled_path = analyze_video(tmp.name, model, detection_mode=detection_mode)
